...
此外，C还定义了 `ptrdiff_t` 类型和`t`修饰符来表示系统使用的两个地址差值的底层有符号整数类型。

## 使用可移植的类型

C 提供了很多整数类型，但是某些类型在不同系统中表现不一致，C99 新增了两个头文件`<stdint.h>, <inttypes.h>`，以确保 C 语言类型在各个系统中表现一致。在实际的开发中，推荐使用这些可移植的类型。

**精确宽度整数类型（exact-width integer type）**：这些新的类型名定义在`stdint.h`头文件中。例如，`int32_t` 表示32位的有符号整数类型。在使用32位int的系统中，头文件会把`int32_t`作为int的别名。

**最小宽度类型（minimum width type）**：

- 对于精确宽度整数类型，计算机的底层系统可能不支持。因此，精确宽度整数类型是可选项。
- C99和C11提供了第2类别名集合。一些类型名保证所表示的类型一定是至少有指定宽度的最小整数类型。这组类型集合被称为最小宽度类型。
- 例如，`int_least8_t` 是可容纳8位有符号整数值的类型中宽度最小的类型的一个别名。

**最快最小宽度类型（fastst minimum width type）**：

- 一些程序员更关心速度而非空间。为此，C99和C11定义了一组可使计算达到最快的类型集合。
- 例如，int_fast8_t被定义为系统中对8位有符号值而言运算最快的整数类型的别名。

**最大整数类型**：有些程序员需要系统的最大整数类型。为此，C99定义了最大的有符号整数类型`intmax_t`，可储存任何有效的有符号整数值。类似地，`unitmax_t`表示最大的无符号整数类型。

以上这些类型都以 _t 结尾，可以理解为都是通过 typedefine 来定义的。
