...
**说明情况下需要关心 ABA 问题**？可能大多数情况下我们并不关心 ABA 问题，例如数值的原子递增，但也不能所有情况下都不关心，例如原子化的更新对象很可能就需要关心 ABA 问题，因为两个 A 虽然相等，但是第二个 A 的属性可能已经发生变化了。所以在使用 CAS 方案的时候，一定要先 check 一下。

## 3 Java 如何实现原子化的 `count += 1`

JDK 中的原子类 AtomicLong 的 `getAndIncrement()` 方法内部就是基于 CAS 实现的，我们来看看 Java 是如何使用 CAS 来实现原子化的 `count += 1` 的。
