package me.ztiany.io.okio.official;
...
public final class SocksProxyServer {
    ...
    public Proxy proxy() {
        return new Proxy(Proxy.Type.SOCKS, InetSocketAddress.createUnresolved("localhost", serverSocket.getLocalPort()));
    }
    ...
    private void handleSocket(final Socket fromSocket) {
        try {
            ...
            // Read the hello.
            int socksVersion = fromSource.readByte() & 0xff;//转换为无符号类型
